...
class App(tk.Frame):
    def __init__(self, master):
        ...
        master.bind("<Right>", self.go_left)
        master.bind("<Left>", self.go_right)
        master.bind("<Down>", self.go_up)
        master.bind("<Up>", self.go_down)
        master.bind("<space>", self.space_key)
        master.bind("<.>", self.go_b)
        master.bind("</>", self.go_v)
    ...
    def go_b(self, event):
        ...
        x, y = self.canvas.coords(self.ship)[:2]        
        #up
        self.bullets.append(Bullet(self.canvas, x, y, "blue", (0, 8)))
        #left
        self.bullets.append(Bullet(self.canvas, x, y, "blue", (8, 0)))
        #right
        self.bullets.append(Bullet(self.canvas, x, y, "blue", (-8, 0)))
    ...
    def go_v(self, event):
        ...
        x, y = self.canvas.coords(self.ship)[:2]        
        #up
        self.bullets.append(Bullet(self.canvas, x, y, "blue", (0, 5)))
        self.bullets.append(Bullet(self.canvas, x - 50, y, "blue", (0, 5)))
        self.bullets.append(Bullet(self.canvas, x + 50, y, "blue", (0, 5)))
